	...
	<nav class="w-screen fixed bottom-0 left-0 right-0 md:hidden flex flex-col z-20">
		...
		<Transition name="show-menu">
			<div v-if="show_menu_data" class="z-0 absolute bottom-full h-screen w-screen flex flex-col">
				...
				>
					<nav class="menu w-full flex flex-col gap-y-2 px-1">
						<NuxtLink to="/" class="hover:bg-white px-2 rounded dark:hover:bg-alt-dark transition-colors duration-200">
							<div class="flex flex-row items-center gap-4 py-2.5">
								<Icon name="mdi:home-outline" class="text-2xl text-neutral-600 dark:text-c-mode" />
						...
						<NuxtLink to="/products" class="hover:bg-white px-2 rounded dark:hover:bg-alt-dark transition-colors duration-200">
							<div class="flex flex-row items-center gap-4 py-2.5">
								<Icon name="mdi:view-grid-outline" class="text-2xl text-neutral-600 dark:text-c-mode" />
						...
						<NuxtLink to="/shops" class="hover:bg-white px-2 rounded dark:hover:bg-alt-dark transition-colors duration-200">
							<div class="flex flex-row items-center gap-4 py-2.5">
								<Icon name="mdi:shopping-search-outline" class="text-2xl text-neutral-600 dark:text-c-mode" />
								<span class="block text-base">Shops</span>
							</div>
						</NuxtLink>
						<NuxtLink to="/shops" class="hover:bg-white px-2 rounded dark:hover:bg-alt-dark transition-colors duration-200">
							<div class="flex flex-row items-center gap-4 py-2.5">
								<Icon name="mdi:cart-percent" class="text-2xl text-neutral-600 dark:text-c-mode" />
						...
						<NuxtLink to="/shops" class="hover:bg-white px-2 rounded dark:hover:bg-alt-dark transition-colors duration-200">
							<div class="flex flex-row items-center gap-4 py-2.5">
								<Icon name="mdi:arrow-top-right" class="text-2xl text-neutral-600 dark:text-c-mode" />
					...
					<div class="a w-full flex flex-col gap-y-1 border-t border-neutral-200 mt-2 pt-2 dark:border-neutral-600">
						...
						>
							<div class="flex flex-row items-center gap-4 py-2">
								<Icon name="mdi:account-outline" class="text-2xl text-neutral-600 dark:text-c-mode" />
						...
						>
							<Transition mode="out-in">
								<div v-if="colorBg" class="flex flex-row w-full gap-x-3 items-center">
									<Icon name="mdi:white-balance-sunny" class="text-2xl text-neutral-600 dark:text-c-mode" />
									<span class="text-base">Toggle Dark Mode</span>
								</div>
								<div v-else class="flex flex-row w-full gap-x-3 items-center">
									<Icon name="mdi:moon-waning-crescent" class="text-2xl -rotate-45 text-neutral-600 dark:text-c-mode" />
...
#mobi-nav button,
#mobi-nav a {
	@apply relative flex flex-col gap-y-1 items-center justify-center px-2 py-1 text-neutral-700 hover:text-custom dark:text-c-mode dark:hover:text-custom;
}
button:focus small,
button:focus .icon {
